...
class Recommendation(models.Model):
    """Stores a SELL/HOLD/PARTIAL SELL recommendation for a crop lot."""
    ...
    crop_lot = models.ForeignKey(
        CropLot, on_delete=models.CASCADE, related_name="recommendations"
    )
    best_market = models.ForeignKey(
    ...
    expected_future_net_value = models.DecimalField(
        max_digits=14, decimal_places=2, null=True, blank=True
    )
    sell_quantity_suggestion = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True
    )
    confidence = models.FloatField(null=True, blank=True)
    reasoning_factors = models.JSONField(default=dict)
    rule_version = models.CharField(max_length=20, default="1.0")
    ...
    def __str__(self):
        return (
            f"{self.crop_lot} → {self.recommendation} "
            f"(₹{self.current_net_value})"
        )
