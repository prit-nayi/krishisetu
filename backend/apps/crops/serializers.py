...
class CropLotSerializer(serializers.ModelSerializer):
    """Read serializer for CropLot — includes display fields."""
    ...
    class Meta:
        model  = CropLot
        fields = (
            "id", "commodity", "commodity_display", "variety",
            ...
            "storage_start_date", "notes", "is_active", "created_at", "updated_at",
        )
        read_only_fields = ("id", "created_at", "updated_at")


class CropLotCreateUpdateSerializer(serializers.ModelSerializer):
    """Write serializer for CropLot creation and updates."""

    class Meta:
        model  = CropLot
        fields = (
            "commodity", "variety", "quantity", "unit",
            "moisture_percent", "quality_grade", "harvest_date",
            "storage_status", "storage_start_date", "notes",
        )
    ...
    def validate_moisture_percent(self, value):
        if value is not None and not (0 <= value <= 100):
            raise serializers.ValidationError("Moisture percent must be between 0 and 100.")
        return value
