...
class CropLot(models.Model):
    ...
    farmer = models.ForeignKey(
        ...
        related_name="crop_lots",
    )
    commodity = models.CharField(max_length=20, choices=Commodity.choices)
    variety = models.CharField(max_length=100, blank=True, null=True)
    quantity = models.DecimalField(
        max_digits=10, decimal_places=2,
        help_text="Quantity in the specified unit",
    )
    unit = models.CharField(
        max_length=10, choices=Unit.choices, default=Unit.QUINTAL
    )
    ...
    quality_grade = models.CharField(
        max_length=5, choices=QualityGrade.choices, null=True, blank=True
    )
    harvest_date = models.DateField()
    storage_status = models.CharField(
        max_length=20, choices=StorageStatus.choices, default=StorageStatus.FARM
    )
    ...
    def __str__(self):
        return (
            ...
            f"{self.get_commodity_display()} {self.quantity}{self.unit}"
        )
    ...
    def clean(self):
        ...
        if (
            self.moisture_percent is not None
            and not (0 <= self.moisture_percent <= 100)
        ):
            raise ValidationError(
                {"moisture_percent": "Moisture percent must be between 0 and 100."}
            )
